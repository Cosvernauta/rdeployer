...
then
    ...
    echo "===== UNDEPLOY APP =====" >> ${APNAME}.log
    nohup ${JAVA_HOME}/bin/java -Xms512M -Xmx512M -cp ${WEBLOGIC_HOME}/server/lib/weblogic.jar weblogic.Deployer ${OPTIONS} -undeploy >> ${APNAME}.log 2>&1
...
echo "===== DEPLOY/REDEPLOY APP =====" >> ${APNAME}.log
nohup ${JAVA_HOME}/bin/java -Xms512M -Xmx512M -cp ${WEBLOGIC_HOME}/server/lib/weblogic.jar weblogic.Deployer ${OPTIONS} -redeploy >> ${APNAME}.log 2>&1

if [ $? -gt 0 ]
then
    msg "Error en la instalación del componente, verificar la salida en el archivo ${APNAME}.log en workspace" "ERROR"
    ...
    exit 1
        ...
        echo "===== STOP APP =====" >> ${APNAME}.log
        nohup ${JAVA_HOME}/bin/java -Xms512M -Xmx512M -cp ${WEBLOGIC_HOME}/server/lib/weblogic.jar weblogic.Deployer ${OPTIONS} -stop >> ${APNAME}.log 2>&1
        msg "Componente Apagado" "OK"

        msg "Encendiendo el componente:" "INFO" # 110119-0528
        echo "===== START APP =====" >> ${APNAME}.log
        nohup ${JAVA_HOME}/bin/java -Xms512M -Xmx512M -cp ${WEBLOGIC_HOME}/server/lib/weblogic.jar weblogic.Deployer ${OPTIONS} -start >> ${APNAME}.log 2>&1
